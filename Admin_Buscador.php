...
<div align="right"><a href="https://www.fi.upm.es/" target="_blank"><img src="imagenes/logo_etsiinf_transparente.png"></a></div>
	...
	if (isset($_COOKIE["mail"]) && isset($_COOKIE["id_sesion"])) {
		if ($_COOKIE["mail"] != "" && $_COOKIE["id_sesion"] != "") {
			...
			if ($result && $row != [] && $row["id_sesion"] == md5($_POST["mail"] . "" . $_SERVER['REMOTE_ADDR'])) {
				...
				$recibir_busqueda_profesor_admin = "recibir_busqueda_profesor_admin.php";
			} else {
	?>
		...
		<?php
				// $recibir_busqueda_profesor_admin = "Login.php";
	...
	<div class="main-container">
		...
		<form action=<?php echo $recibir_busqueda_profesor_admin ?> method="POST">
			<p class="generalseparator">Nombre: <input type="text" name="nombre" required></p>
			<input type="hidden" name="mail" value=<?php echo $_POST["mail"] ?>>
			<input type="hidden" name="id" value=<?php echo $_POST["id"] ?>>
			<input type="submit" name="Buscar" value="Buscar Profesor" class="generalseparator functionality-button">
		</form>
