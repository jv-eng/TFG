...
<div align="right"><a href="https://www.fi.upm.es/" target="_blank"><img src="imagenes/logo_etsiinf_transparente.png"></a></div>
	...
	<div class="main-container">
		...
		if ($id_profesor != "") {
			...
			$query = $con->prepare("INSERT INTO `franja_disponibilidad` VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?);");
			mysqli_stmt_bind_param($query, "issiiisis", $id_profesor,$_POST["asignatura"],$tipo,
				$_POST["hora"],$_POST["minutos"],$_POST["duracion_slots"],$_POST["dia"],$_POST["numero_slots"],$_POST["ubicacion"]);
			mysqli_stmt_execute($query);
			$result = mysqli_stmt_get_result($query);
			mysqli_stmt_close($query);
			...
			while ($numero_slots > 0) {
				...
				mysqli_stmt_bind_param($query, "iiiis", $idfranja,$hora,$minutos,$duracion_slots,$_POST["dia"]);
				mysqli_stmt_execute($query);
				$result = mysqli_stmt_get_result($query);
				mysqli_stmt_close($query);
