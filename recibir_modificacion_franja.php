	...
	<div class="main-container">
		...
		if ($id_profesor != "") {
			...
			if (
				...
				$query = $con->prepare("UPDATE `franja_disponibilidad` 
					SET `hora` = ?, `minutos` = ?, `duracion_slots` = ?, `dia` = ?, `numero_slots` = ?, `ubicacion` = ? WHERE `idfranja` = ?;");
				mysqli_stmt_bind_param($query, "iiisisi", $_POST["hora"], $_POST["minutos"], $_POST["duracion_slots"], $_POST["dia"], $_POST["numero_slots"], $_POST["ubicacion"], $_POST["idfranja"]);
				mysqli_stmt_execute($query);
				$result1 = mysqli_stmt_get_result($query);
				mysqli_stmt_close($query);
				...
				while ($numero_slots > 0) {
					...
					$query = $con->prepare($sql = "INSERT INTO `slot` (`id_slot_posicion`, `id_franja_disponibilidad`, `id_alumno_fk`, `hora`, `minutos`, `duracion`, `dia`, `disponible`, `comentarios_alumno`) 
					VALUES (NULL, ?, NULL, ?, ?, ?, ?, '1', NULL)");
