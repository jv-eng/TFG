...
<body onload="todayDate()">
	...
	<div class="main-container">
		...
		if (isset($_POST["id_profesor"])) {
			$idContinuar = $_POST["id_profesor"];
		} else {
			$idContinuar = $_POST["idalumno"];
		}
		if (isset($_POST["BorrarProfesor"]) or isset($_POST["Cancelar"])) {
			$con = mysqli_connect('localhost', 'root', '', 'prueba2_tfg_tutorias');
			...
			*/
			...
			$sql = "SELECT calendarID FROM `profesor` WHERE (`id_profesor` = '" . $_POST["id_profesor"] . "');";
			$result = mysqli_query($con, $sql) or die('Error en la consulta a la BDD');
			if ($result) {
			...
			$sql = "DELETE FROM `profesor` WHERE `id_profesor` = '" . $_POST["id_profesor"] . "'";
			$result = mysqli_query($con, $sql) or die('Error en la consulta a la BDD del profesor');
			if ($result) {
				if (isset($_POST["Cancelar"])) {

		?>
					<h3 class="generalseparator green">Se ha cancelado la cuenta de <?php echo $_POST["mail_profesor"] ?></h3>
				<?php

				} else {
				?>
					<h3 class="generalseparator green">Se ha borrado la cuenta de <?php echo $_POST["mail_profesor"] ?></h3>
				<?php
				...
				?>
					<h3 class="generalseparator red">No se pudo cancelar la cuenta de <?php echo $_POST["mail_profesor"] ?></h3>
				<?php

				} else {

				?>
					<h3 class="generalseparator red">No se pudo borrar la cuenta de <?php echo $_POST["mail_profesor"] ?></h3>
				<?php
				}
			}
			mysqli_close($con);
			...
			$con = mysqli_connect('localhost', 'root', '', 'prueba2_tfg_tutorias');
			$sql = "DELETE FROM `alumno` WHERE `idalumno` = '" . $_POST["idalumno"] . "'";
			$result = mysqli_query($con, $sql) or die('Error en la consulta a la BDD del alumno');
			if ($result) {

				?>
				<h3 class="generalseparator green">Se ha borrado la cuenta de <?php echo $_POST["mail_alumno"] ?></h3>
			...
			?>
				<h3 class="generalseparator red">No se pudo borrar la cuenta de <?php echo $_POST["mail_alumno"] ?></h3>
		<?php
		...
		<form action=<?php echo $Continuar ?> method="POST" class="generalseparator">
			...
			<input type="hidden" name="id" value=<?php echo $idContinuar ?>>
